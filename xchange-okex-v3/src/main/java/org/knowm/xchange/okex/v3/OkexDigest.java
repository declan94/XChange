...
public class OkexDigest extends BaseParamsDigest implements ParamsDigest {
    ...
    public String digestParams(RestInvocation restInvocation) {
        StringBuilder messageBuilder = new StringBuilder(restInvocation.getHttpHeadersFromParams().get(OkexAuthenticated.ACCESS_TIMESTAMP_PARM));
        messageBuilder.append(restInvocation.getHttpMethod())
                .append("/").append(restInvocation.getPath());
        ...
        return Base64.getEncoder().encodeToString(getMac().doFinal(messageBuilder.toString().getBytes()));
    }
